...
def max_flow(cap, s, t):
    ...
    # Create the objective function for the LP
    c = [-1 if i < n else 0 for i in range(n*n)]  # Minimize the flow from the source node
    
    # Create equality constraints for flow conservation at each node
    template = [0] * (n*n)
    A_eq = []
    for i in range(n):
        if i == t or i == s:
    ...
    b_eq = [0]*len(A_eq)  # Flow into the sink node should be 1

    # Bounds for each variable (flow on each edge)
    bounds = []
    for i in range(n):
    	for j in range(n):
    ...
    # Extract the flows from the solution
    flows_per_edge = [[0]*n for _ in range(n)]
    for i in range(n):
        for j in range(n):
            flows_per_edge[i][j] = opt.x[i*n+j]
    
    # Total flow is the flow into the sink node
